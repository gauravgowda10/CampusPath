...
public class Box implements Iterable<Ball> {
    ...
    /**
     * maxVolume is used to internally store the maximum volume for this box
     */
    ...
    public boolean add(Ball b) {
        // Your code goes here.  Remove the exception after you're done.
        if(ballContainer.getVolume() + b.getVolume() <= maximumVolume && !ballContainer.contains(b)) {
           ballContainer.add(b);
           return true;
        }
        return false;
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        ArrayList<Ball> orderedList = new ArrayList<Ball>();
        Iterator<Ball> ballIterator = ballContainer.iterator();

        while(ballIterator.hasNext()){
            orderedList.add(ballIterator.next());
        }
        orderedList.sort(Comparator.comparingDouble(Ball::getVolume));
