...
public final class RatPoly {
    ...
    private static void scaleCoeff(List<RatTerm> lst, RatNum scalar) {
        // {{inv: list = list(0), list(1)....list(i-1) where j is the j term in the list}}
        for(int i = 0; i < lst.size(); i++) {
            RatNum newCoeff = lst.get(i).getCoeff().mul(scalar);
            RatTerm newTerm = new RatTerm(newCoeff, lst.get(i).getExpt());
            lst.set(i, newTerm);
        }
    }
    ...
    private static void incremExpt(List<RatTerm> lst, int degree) {
        // {{inv: lst(0), lst....lst(i-1) where lst(j) is the j term in the list}}
        for (int i = 0; i < lst.size(); i++) {
    ...
    private static void sortedInsert(List<RatTerm> lst, RatTerm newTerm) {
        if (!newTerm.isZero()) {
            // {{inv: lst.get(0) > list(1) ... > lst.get(i-1), where lst(j) is the jth term in lst}}
            for (int i = 0; i < lst.size(); i++) {
    ...
    public RatPoly negate() {
        ...
        List<RatTerm> negation = new ArrayList<RatTerm>(this.terms);
        RatNum negativeOne = new RatNum(-1);
        scaleCoeff(negation, negativeOne);
        return new RatPoly (negation);
    }
    ...
    public RatPoly add(RatPoly p) {
        ...
        List<RatTerm> sum = new ArrayList<RatTerm>(this.terms);
        // {{inv: sum = this.terms + p.terms_0 +...+ p.terms(i-1) where p_j is the jth term in p.}}
        for (int i = 0; i < p.terms.size(); i++) {
            sortedInsert(sum, p.terms.get(i));
        }
        return new RatPoly(sum);
    }
    ...
    public RatPoly mul(RatPoly p) {
        ...
        // {{inv: product = (this.terms(0)+...+this.terms(i-1)) * (p.terms(0)+...+p.terms(i-1)), where
        // this.terms(x) is the xth term in this.terms and p(y) is the yth term in p}}
    ...
    public RatPoly div(RatPoly p) {
        ...
        RatPoly result = new RatPoly(this.terms);
        // {{inv: quotient = result(0)/p(0) + result(1)/p(1) +...+result(i-1)/p(i-1)}
        // where result(0) represents the first term of the result polynomial at the
        // start of each iteration through the loop.}}
        while(!result.terms.isEmpty() && result.degree() >= p.degree()) {
            RatPoly tempPoly = new RatPoly(result.terms.get(0).div(p.terms.get(0)));
            quotient = quotient.add(tempPoly);
            result = result.sub(p.mul(tempPoly));
        }
        return quotient;
    }
    ...
    public RatPoly differentiate() {
        ...
        List<RatTerm> derivative = new ArrayList<>();
        // {{inv: derivative = this.terms(0).differentiate, this.terms(1).differentiate,
        // ...,list(i-1) where this.terms(j) is the j term in the list}}
        for (int i = 0; i < terms.size(); i++) {
            if (!terms.get(i).differentiate().isZero()) {
                derivative.add(terms.get(i).differentiate());
            }
        }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        ...
        List<RatTerm> integral = new ArrayList<>();
        // {{inv: integral = this.terms(0).antiDifferentiate, this.terms(1).antiDifferentiate,
        // ...,list(i-1).antiDifferentiate where this.terms(j) is the j term in the list}}
        for (int i = 0; i < this.terms.size(); i++) {
            integral.add(this.terms.get(i).antiDifferentiate());
        }
        if (!integrationConstant.equals(RatNum.ZERO)) {
            sortedInsert(integral, new RatTerm(integrationConstant, 0));
        }
        return new RatPoly(integral);
    }
